...
func (s *KeeperTestSuite) TestApplyTransaction() {
	...
	err := s.Network.App.GetBankKeeper().MintCoins(ctx, "mint", sdk.NewCoins(sdk.NewCoin(testconstants.ExampleAttoDenom, sdkmath.NewInt(3e18))))
	s.Require().NoError(err)
	err = s.Network.App.GetBankKeeper().SendCoinsFromModuleToModule(ctx, "mint", "fee_collector", sdk.NewCoins(sdk.NewCoin(testconstants.ExampleAttoDenom, sdkmath.NewInt(3e18))))
	s.Require().NoError(err)
	// With virtual fee collection enabled, RefundGas uses virtual balance.
	// Move the fee collector's real coins into its virtual balance.
	feeCollectorAddr := authtypes.NewModuleAddress(authtypes.FeeCollectorName)
	err = s.Network.App.GetBankKeeper().SendCoinsFromAccountToModuleVirtual(
		ctx, feeCollectorAddr, authtypes.FeeCollectorName, sdk.NewCoins(sdk.NewCoin(testconstants.ExampleAttoDenom, sdkmath.NewInt(3e18))),
	)
	s.Require().NoError(err)
	testCases := []struct {
		name       string
	...
	for _, tc := range testCases {
		s.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			s.Require().NoError(err)
			initialBalance := s.Network.App.GetBankKeeper().GetBalance(ctx, s.Keyring.GetAccAddr(0), testconstants.ExampleAttoDenom)
			...
			balanceAfterRefund := s.Network.App.GetBankKeeper().GetBalance(ctx, s.Keyring.GetAccAddr(0), testconstants.ExampleAttoDenom)
...
func (s *KeeperTestSuite) TestApplyTransactionWithTxPostProcessing() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(s *KeeperTestSuite) {
				senderBefore := s.Network.App.GetBankKeeper().GetBalance(s.Network.GetContext(), s.Keyring.GetAccAddr(0), testconstants.ExampleAttoDenom).Amount
				recipientBefore := s.Network.App.GetBankKeeper().GetBalance(s.Network.GetContext(), s.Keyring.GetAccAddr(1), testconstants.ExampleAttoDenom).Amount
				...
				senderAfter := s.Network.App.GetBankKeeper().GetBalance(s.Network.GetContext(), s.Keyring.GetAccAddr(0), testconstants.ExampleAttoDenom).Amount
				recipientAfter := s.Network.App.GetBankKeeper().GetBalance(s.Network.GetContext(), s.Keyring.GetAccAddr(1), testconstants.ExampleAttoDenom).Amount
...
func (s *KeeperTestSuite) TestApplyMessageWithNegativeAmount() {
	...
	ctx := s.Network.GetContext()
	balance0Before := s.Network.App.GetBankKeeper().GetBalance(ctx, s.Keyring.GetAccAddr(0), testconstants.ExampleAttoDenom)
	balance1Before := s.Network.App.GetBankKeeper().GetBalance(ctx, s.Keyring.GetAccAddr(1), testconstants.ExampleAttoDenom)
	stateDB := statedb.New(s.Network.GetContext(), s.Network.App.GetEVMKeeper(), statedb.NewEmptyTxConfig())
	...
	balance0After := s.Network.App.GetBankKeeper().GetBalance(ctx, s.Keyring.GetAccAddr(0), testconstants.ExampleAttoDenom)
	balance1After := s.Network.App.GetBankKeeper().GetBalance(ctx, s.Keyring.GetAccAddr(1), testconstants.ExampleAttoDenom)
